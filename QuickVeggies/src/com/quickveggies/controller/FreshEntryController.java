...
public class FreshEntryController implements Initializable {
	...
	private final ArrayList<TextFieldTableCell<?, ?>> growerLotColList = new ArrayList<>();
	...
	List<TableColumn> boxesList = new ArrayList<TableColumn>();
						...
						autCompCellBuyerSelect.setOnKeyReleased((KeyEvent event) -> {
							if (event.getCode() == KeyCode.ENTER) {
								...
								if (selectedBuyer == null || selectedBuyer.trim().isEmpty()
								 ...
								 System.out.println("The row number for select buyer field:" + row);
...
//				});
		...
		buyersEntry.getColumns().addAll(tcBuyerSelect, tcBuyerQty);
					...
					if (focusedCell != null) {
						...
						if (focusedCell.getTableColumn() == tcBuyerSelect) {
							...
							boxesList.forEach(box -> {
								if (focusedCell.getTableColumn() == box) {
									buyersEntry.edit(row, box);
									if (shouldFireTabKey) {
										shouldFireTabKey = false;
										fireKeyPress(java.awt.event.KeyEvent.VK_TAB);
									}
		...
		for (int i = 0; i < boxSizesList.size(); i++ ){
			
		
			final TableColumn tcBuyerRatePBox = new TableColumn(boxSizesList.get(i));
			tcBuyerRatePBox.setCellValueFactory(
					new PropertyValueFactory<FreshEntryTableData.GrowerEntryTableLine, String>("quantity"+(i)));
			tcBuyerRatePBox.setCellFactory(TextFieldTableCell.forTableColumn());
			tcBuyerRatePBox.setOnEditCommit(createTableCellFieldEventHandler("quantity"+(i)));
			buyersEntry.getColumns().add(tcBuyerRatePBox);
			boxesList.add(tcBuyerRatePBox);
		};
 
		// try {
		// parent =
		// FXMLLoader.load(getClass().getResource("/fxml/freshentry.fxml"));
	...
	// -----------------------------------------------
	public void createGrowerTable() {
		growersEntry.getColumns().clear();
		...
		TableColumn tcGrowerLot = new TableColumn("Lot");
		tcGrowerLot.setCellFactory(
				new javafx.util.Callback<TableColumn<FreshEntryTableData.GrowerEntryTableLine, String>, TableCell<FreshEntryTableData.GrowerEntryTableLine, String>>() {
					@Override
					public TableCell<FreshEntryTableData.GrowerEntryTableLine, String> call(
							TableColumn<FreshEntryTableData.GrowerEntryTableLine, String> param) {
		...
		tcGrowerLot
				.setCellValueFactory(new PropertyValueFactory<FreshEntryTableData.GrowerEntryTableLine, String>("lot"));
		tcGrowerLot.setEditable(false);
		...
		growersEntry.getColumns().addAll(tcGrowerLot, tcGrowerQual,tcGrowerQty);
		growersEntry.setEditable(true);
		
		for (int i = 0; i < boxSizesList.size(); i++ ){
			
		
			final TableColumn tcGrowerRatePBox = new TableColumn(boxSizesList.get(i));
			tcGrowerRatePBox.setCellValueFactory(
					new PropertyValueFactory<FreshEntryTableData.GrowerEntryTableLine, String>("quantity"+(i)));
			tcGrowerRatePBox.setCellFactory(TextFieldTableCell.forTableColumn());
			tcGrowerRatePBox.setOnEditCommit(createTableCellFieldEventHandler("quantity"+(i)));
			growersEntry.getColumns().add(tcGrowerRatePBox);
			boxesList.add(tcGrowerRatePBox);
		};

		// final TableColumn tcGrowerRatePBox = new TableColumn("Rate Per Box");
		// tcGrowerRatePBox.setCellValueFactory(
		// new PropertyValueFactory<FreshEntryTableData.GrowerEntryTableLine,
		// String>("growerRate"));
		// tcGrowerRatePBox.setCellFactory(TextFieldTableCell.forTableColumn());
		...
		growersEntry.setOnKeyReleased(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				if (event.getCode() == KeyCode.ENTER) {
					...
					if (focusedCell != null) {
						int row = focusedCell.getRow();
						if (focusedCell.getTableColumn() == tcGrowerQty) {
							growersEntry.edit(row, tcGrowerQty);
						} else {
							boxesList.forEach(box -> {
								if (focusedCell.getTableColumn() == box) {
									 System.out.println("In Grower rate box column cell" + box.getText());
									growersEntry.edit(row, box);
									if (shouldFireTabKey) {
										shouldFireTabKey = false;
										fireKeyPress(java.awt.event.KeyEvent.VK_TAB);
									}
...
//					return;
//				}
				System.out.println(tv + "--------" + isLastColumn);
				if (isLastRow) {
	...
	private TreeSet<String> updateGrowersList() {
		...
		try {
			result = new TreeSet<String>(
					dbclient.getSuppliers().stream().map(supplier -> supplier.getTitle()).collect(Collectors.toList()));
	...
	public void keyPressed(KeyCode key) {
		if (key == KeyCode.ENTER) {
					...
					if (currInd < traversibleNodes.size()) {
						...
						if (currentNode == grower) {
							...
							if (selectedSupplier == null || selectedSupplier.trim().isEmpty()
									|| !growersList.contains(selectedSupplier.trim())) {
			...
			// checkQuantities(); // only one table can be enabled at any
			// moment(the one being // edited)
			// String emptyFieldMsg = "Can't continue-current table hasn't been
			// properly filled!";
			// if (currTable == LOT) {
			 ...
			 * }
			 */ /*
				 * GeneralMethods.errorMsg(emptyFieldMsg); else { // if the
				 * entry is a regular one, continue to populate the // buyers
				 * table if (freshEntryType == FreshEntryController.REGULAR) {
				 * buyersEntry.setEditable(true); buyersEntry.requestFocus();
				 * currTable = BUYERS; } // if the entry is coldstore/godown,
				 * skip the buyers and // check the growers else { // check if
				 * more grower lines are needed if (totalGrowersQty <
				 ...
				 * "")); updateTable(growersEntry, growerLines);
				 * growersEntry.setEditable(false); // check if more lot lines
				 * are needed if (totalLotQty <
				 * Integer.parseInt(totalQty.getText())) {
				 * addTableLine(addLotLines, data.getAddLotTableLine("", "0",
				 * "")); // addLots.setEditable(true); addLots.requestFocus();
				 * currTable = LOT; } } } }
				 * 
				 ...
				 * checkQuantities(); // check if more buyers lines are needed
				 * if (totalBuyersQty <
				 ...
				 * buyer,grower,buyerqty,quality,boxsize,buyerRate,totalProfit
				 * for (FreshEntryTableData.BuyerEntryTableLine line :
				 * buyerLines) { FreshEntryTableData.buyerEntryLinesSql .add(new
				 * String[] { line.getBuyerSelect(), grower.getText(),
				 * line.getBuyerQty(), addLotLines.get(addLotLines.size() -
				 * 1).getQualityType(), growerLines.get(growerLines.size() -
				 * 1).getBoxSize(), line.getBuyerRate(), "" +
				 * (Integer.parseInt(line.getBuyerRate())
				 * Integer.parseInt(line.getBuyerQty())) }); }
				 * buyerLines.clear();
				 * buyerLines.add(data.getBuyerEntryTableLine("", "0", "0",
				 * "")); // data.disableCommittedLines(growerLines);
				 * updateTable(buyersEntry, buyerLines);
				 * buyersEntry.setEditable(false); // check if more grower lines
				 * are needed if (totalGrowersQty <
				 * Integer.parseInt(addLotLines.get(addLotLines.size() -
				 * 1).getQty())) { addTableLine(growerLines,
				 * data.getGrowerEntryTableLine("", "0", "0", ""));
				 * growersEntry.requestFocus(); currTable = GROWERS; } else { //
	...
	private AutoCompleteTextField createAutoCompleteBuyerTextField() {
		...
		buyerSelectTextField.linkToWindow(FreshEntryController.this, "/fxml/buyeradd.fxml", "Add new buyer",
	 ...
	 *
	 * @param tv
	 *            - Table which gets focus
	 * @param tc
	 *            - Column of the table to have the focus
	 */
	private <T> void setFocusListenerForTableColumn(final TableView<T> tv, final TableColumn<T, String> tc) {
		tv.focusedProperty()
				.addListener((ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) -> {
					...
					if ((columnIndex + 1) < totalCols) {
						...
						TableColumn<?, String> nextColInSeq = (TableColumn<?, String>) tv.getColumns()
								.get(columnIndex + 1);
	...
	private int handleLastColOfGrowerTable(TableView<FreshEntryTableData.GrowerEntryTableLine> tv, int curRow,
			TableColumn.CellEditEvent event) {
		event.consume();
System.out.println("Last column of grower Table");
	...
	private void addNewGrowersEntry(TableView<FreshEntryTableData.GrowerEntryTableLine> growerTable,
		...
		GrowerEntryTableLine growerEntryTableLine = data.new GrowerEntryTableLine(sessionController.fruitType+"/"+String.valueOf(remainderQty), "",
				String.valueOf(remainderQty), "", qualType);
		int totalboxes = boxesList.size();
		if(1 <= totalboxes) {
			growerEntryTableLine.setQuantity0("0");
			System.out.println("1");
		}
		if(2 <= totalboxes) {
			growerEntryTableLine.setQuantity1("0");
			System.out.println("2");
		}
		if(3 <= totalboxes) {
			growerEntryTableLine.setQuantity2("0");
			System.out.println("3");
		}
		if(4 <= totalboxes) {
			growerEntryTableLine.setQuantity3("0");
			System.out.println("4");
		}
		if(5 <= totalboxes) {
			growerEntryTableLine.setQuantity4("0");
			System.out.println("5");
		}
		if(6 <= totalboxes) {
			growerEntryTableLine.setQuantity5("0");
			System.out.println("6");
		}
		if(7 <= totalboxes) {
			growerEntryTableLine.setQuantity6("0");
			System.out.println("7");
		}
				
		GrowerEntryTableLine growerEntryTableLine2 = data.new GrowerEntryTableLine("Rate", "",
				"0", "", "----->");

		
		growerTable.getItems().add(growerEntryTableLine);
		growerTable.getItems().add(growerEntryTableLine2);
	...
	private void addNewBuyersEntry(TableView<FreshEntryTableData.BuyerEntryTableLine> buyerTable,
		...
		BuyerEntryTableLine buyerTableLine = data.new BuyerEntryTableLine("", String.valueOf(remainderQty), "", buyerBoxSizeType);

		BuyerEntryTableLine buyerTableLine1 = data.new BuyerEntryTableLine("Rate", "", "", buyerBoxSizeType);
		
	
		int totalboxes = boxesList.size();
		if(1 <= totalboxes) {
			buyerTableLine.setQuantity0("0");
			buyerTableLine1.setQuantity0("0");
			System.out.println("1");
		}
		if(2 <= totalboxes) {
			buyerTableLine.setQuantity1("0");
			buyerTableLine1.setQuantity1("0");
			System.out.println("2");
		}
		if(3 <= totalboxes) {
			buyerTableLine.setQuantity2("0");
			buyerTableLine1.setQuantity2("0");
			System.out.println("3");
		}
		if(4 <= totalboxes) {
			buyerTableLine.setQuantity3("0");
			buyerTableLine1.setQuantity3("0");
			System.out.println("4");
		}
		if(5 <= totalboxes) {
			buyerTableLine.setQuantity4("0");
			buyerTableLine1.setQuantity4("0");
			System.out.println("5");
		}
		if(6 <= totalboxes) {
			buyerTableLine.setQuantity5("0");
			buyerTableLine1.setQuantity5("0");
			System.out.println("6");
		}
		if(7 <= totalboxes) {
			buyerTableLine.setQuantity6("0");
			buyerTableLine1.setQuantity6("0");
			System.out.println("7");
		}
		

		buyerTable.getItems().add(buyerTableLine);
		buyerTable.getItems().add(buyerTableLine1);
